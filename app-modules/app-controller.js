...
function useAppController(){
  ...
  function restoreSnapshot(snapshot){
    ...
    var snapUi=snapshot&&snapshot.ui;
    if(snapUi&&snapUi.iqOptions&&typeof setIqOptions==="function")setIqOptions(snapUi.iqOptions);
    if(snapUi&&snapUi.audioFftOptions&&typeof setAudioFftOptions==="function")setAudioFftOptions(snapUi.audioFftOptions);
  }
  ...
  function copyChartPng(){
    try{
      if(typeof svgMarkupToPngBlob!=="function"){setError("Copy image is not available in this build.");return;}
      var exportAsset=buildChartVectorExport({kind:"png"});
      svgMarkupToPngBlob(exportAsset.markup,exportAsset.width,exportAsset.height,{scale:2,backgroundColor:exportAsset.backgroundColor}).then(function(blob){
        if(!navigator.clipboard||!navigator.clipboard.write){
          setError("Clipboard API not available — use PNG download instead.");
          return;
        }
        var item=new ClipboardItem({"image/png":blob});
        return navigator.clipboard.write([item]);
      }).then(function(){
        setError("Chart image copied to clipboard.");
      }).catch(function(err){
        setError("Copy failed: "+(err&&err.message?err.message:"Unknown error.")+" — try PNG download instead.");
      });
    }catch(err){
      setError("Unable to copy chart: "+(err&&err.message?err.message:"Unknown error."));
    }
  }
  function retuneFmForFile(fileId,offsetHz){
    var f=(files||[]).find(function(x){return x&&x.id===fileId;});
    if(!f||!f.iqRaw||typeof PH.demodulateIqFmAtOffset!=="function")return;
    var raw=f.iqRaw;
    try{
      var result=PH.demodulateIqFmAtOffset(raw.I,raw.Q,raw.sampleRate,offsetHz,Object.assign({},audioFftOptions||{},iqOptions||{}));
      setFiles(function(prev){
        return (prev||[]).map(function(file){
          if(!file||file.id!==fileId)return file;
          var newTraces=(file.traces||[]).map(function(tr){
            if(tr&&tr.kind==="derived"&&String(tr.name||"").indexOf("FM demod")>=0){
              return Object.assign({},tr,{data:result.data||[]});
            }
            return tr;
          });
          var newFmPcm=result.pcmSamples||null;
          var newDemodPcm=file.iqDemodPcm?Object.assign({},file.iqDemodPcm,{fmSamples:newFmPcm}):file.iqDemodPcm;
          return Object.assign({},file,{traces:newTraces,iqDemodPcm:newDemodPcm});
        });
      });
    }catch(e){setError("FM retune failed: "+(e&&e.message||e));}
  }
  function downloadIqAudio(fileId,mode){
    var f=(files||[]).find(function(x){return x&&x.id===fileId;});
    if(!f||!f.iqDemodPcm||typeof buildWavBlob!=="function")return;
    var samples=mode==="fm"?f.iqDemodPcm.fmSamples:f.iqDemodPcm.amSamples;
    if(!samples||!samples.length)return;
    var rate=f.iqDemodPcm.rate||48000;
    try{
      var blob=buildWavBlob(samples,Math.round(rate));
      var baseName=String(f.fileName||"iq").replace(/\.[^.]+$/,"");
      downloadBlobFile(blob,baseName+"-"+(mode||"am")+".wav");
    }catch(e){setError("WAV export failed: "+(e&&e.message||e));}
  }
  function ztrLoadFiles(entries){
    if(!entries||!entries.length)return;
    var fileObjs=entries.map(function(e){
      return new File([e.buffer],e.name);
    });
    loadFiles(fileObjs,true);
  }
  ...
  var importExportPanelProps={visible:showImportExportPanel,C:C,hasData:hasData,files:files,allTr:allTr,panes:panes,activePaneId:activePaneId,fRef:fRef,iRef:iRef,openWorkspace:openWorkspace,exportWorkspace:exportWorkspace,exportTraceData:exportTraceData,exportChartPng:exportChartPng,copyChartPng:copyChartPng,exportChartSvg:exportChartSvg,exportChartCsv:exportChartCsv,shareWorkspaceLink:shareWorkspaceLink,audioFftOptions:audioFftOptions,setAudioFftOptions:setAudioFftOptions,audioFftWindows:PH.AUDIO_WINDOWS||{},audioFftSizes:PH.AUDIO_FFT_SIZES||[],iqOptions:iqOptions,setIqOptions:setIqOptions,audioFiles:audioFilesList,runCrossSourceCompare:runCrossSourceCompare,crossSourceReport:crossSourceReport,clearCrossSourceReport:function(){setCrossSourceReport(null);},streamCaptureUrl:streamCaptureUrl,setStreamCaptureUrl:setStreamCaptureUrl,streamCaptureBusy:streamCaptureBusy,captureFromStreamUrl:captureFromStreamUrl,clearAllFiles:clearAllFiles,onZtrLoad:ztrLoadFiles};
